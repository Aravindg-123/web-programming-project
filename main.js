...
function showPage(key) {
  ...
  const target = document.getElementById(pageMap[key] || 'pageDashboard');
  if (target) target.style.display = key === 'dashboard' ? 'grid' : 'flex';

  // init sub-page charts if needed
  if (key === 'quotas' && !subChartsInited.quotas)   { initQuotaCharts(); subChartsInited.quotas = true; }
  if (key === 'alerts' && !subChartsInited.alerts)   { initAlertCharts(); subChartsInited.alerts = true; }
  if (key === 'seasonal' && !subChartsInited.seasonal){ initSeasonalCharts(); subChartsInited.seasonal = true; }
}

const subChartsInited = {};
